...
| [Rewrite HardsubX subsystem in Rust](/public/gsoc/2022/hardsubx)  | Hardsubx is used to extract subtitles that are burned into videos using OCR. We have a implementation in C that works OK but has many bugs. We'd like to port it to rust. | (Some) Rust, Some (C), OCR | Tesseract Imaging OCR FFmpeg | Medium |
